...
func (s *Sender) SendBookingConfirmation(to, name string, startTime time.Time, dateTimeNZT, dateTimeLocal string) error {
	...
	icsContent := s.generateICSWithFrom(startTime, endTime, name, "contact@matou.nz")
	...
	msg := s.buildMIMEMessageWithCalendarFrom(to, "Whakawhānaunga Session Booking - MATOU", body, icsContent, "contact@matou.nz")

	addr := fmt.Sprintf("%s:%d", s.host, s.port)
	if err := s.sendMailFrom(addr, "contact@matou.nz", to, []byte(msg)); err != nil {
		return fmt.Errorf("sending email: %w", err)
	}
...
func (s *Sender) generateICSWithFrom(startTime, endTime time.Time, attendeeName, fromEmail string) string {
	...
	b.WriteString("LOCATION:Online (link to be provided)\r\n")
